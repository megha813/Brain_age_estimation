...
from model.HCCT import ViTForClassfication    #HCCT
...
def main(res):
    ...
    if opt.model == 'ScaleDense':
        ...
        model= ViTForClassfication(config=HCCTconfig)
    ...
    loss_func_dict = {'mae': nn.L1Loss().to(device)
                    ...
                    #  ,'ranking':rank_difference_loss(sorter_checkpoint_path=opt.sorter              #i commented out this - as the checkpoint file was not accessible- found out that this loss is not given as input to learning of the
                    #                                 ,beta=opt.beta).to(device)
                     }
        
    criterion1 = loss_func_dict[opt.loss]
    # criterion2 = loss_func_dict[opt.aux_loss]    #commented out as the this is also using the checkpoint and i am not been able to see this as getting used for vit
